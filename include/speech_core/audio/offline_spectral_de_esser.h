...
public:
    ...
    static std::vector<std::vector<double>> process(
        ...
        return process_internal(audio, sample_rate, std::move(params), nullptr);
    }
    ...
    static ProcessResult process_with_report(
        ...
        result.audio = process_internal(audio, sample_rate, std::move(params), &result.report);
        return result;
    }
...
private:
    ...
    static void transform_packed(std::vector<double>& data) {
        validate_packed_fft_buffer(data);

        const std::size_t fft_size = data.size();
        const std::size_t nyquist_bin = fft_size / 2;
        const std::size_t bins = nyquist_bin + 1;

        std::vector<float> input(fft_size, 0.0f);
        std::vector<float> real(bins, 0.0f);
        std::vector<float> imag(bins, 0.0f);

        for (std::size_t i = 0; i < fft_size; ++i)
            input[i] = static_cast<float>(data[i]);

        fft_real(input.data(), fft_size, real.data(), imag.data());

        data[0] = static_cast<double>(real[0]);
        data[1] = static_cast<double>(real[nyquist_bin]);
        for (std::size_t k = 1; k < nyquist_bin; ++k) {
            data[2 * k] = static_cast<double>(real[k]);
            data[2 * k + 1] = static_cast<double>(imag[k]);
        }
    }

    static void inverse_transform_packed(std::vector<double>& data) {
        validate_packed_fft_buffer(data);
        ...
        std::vector<float> real(bins, 0.0f);
        std::vector<float> imag(bins, 0.0f);
        std::vector<float> output(fft_size, 0.0f);

        real[0] = static_cast<float>(data[0]);
        real[nyquist_bin] = static_cast<float>(data[1]);
        for (std::size_t k = 1; k < nyquist_bin; ++k) {
            real[k] = static_cast<float>(data[2 * k]);
            imag[k] = static_cast<float>(data[2 * k + 1]);
        }

        ifft_real(real.data(), imag.data(), fft_size, output.data());
    ...
    static std::vector<double> inverse_stft(
        ...
        for (std::size_t t = 0; t < frames; ++t) {
            ...
            inverse_transform_packed(frame);
