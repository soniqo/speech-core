...
- **ONNX Runtime**（`SPEECH_CORE_WITH_ONNX`）—— Silero VAD、Parakeet STT、Nemotron-3.5 多语言流式 STT、Kokoro TTS、DeepFilterNet3、**PersonaPlex 7B 全双工语音转语音**（CUDA 目标平台）。
...
**硬件加速。** ONNX：Android 上的 NNAPI、Qualcomm Linux 上的 QNN（将 `libQnnHtp.so` 放入库路径）、可选的 NVIDIA CUDA / TensorRT（`-DSPEECH_CORE_WITH_CUDA=ON`）—— 通过 `SPEECH_CORE_ORT_PROVIDER` 在运行时控制，并提供静默的 CPU 回退。LiteRT 目前仅支持 CPU；`libLiteRt` 中的 Hexagon / GPU delegate 已存在但尚未通过 C API 接入。
...
# + ONNX 启用 NVIDIA CUDA / TensorRT (ORT_DIR 必须是启用 GPU 的 ONNX Runtime)
